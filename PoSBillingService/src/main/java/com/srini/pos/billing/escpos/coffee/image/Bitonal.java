...
package com.srini.pos.billing.escpos.coffee.image;
 ...
 * <p>
 * Used on {@link EscPosImage#EscPosImage EscPosImage constructor}. Any dither
 * algorithm can be implemented, like ordered grid or noise dither. Generally,
 * you need to call image.getRGB(x, y) and decide if return zero or one. you
 * need to Override only the {@link #zeroOrOne(int, int, int, int, int, int)
 * zeroOrOne} method but if you want, you can Override
 * {@link #getBitonalVal(java.awt.image.BufferedImage, int, int) getBitonalVal}
 * too.
 */
public abstract class Bitonal {
	/**
	 * Pre-work each coordinate (x, y) of image by discover RGBA values. get the
	 * 8-bits values separated by alpha, red, blue and green and return by calling
	 * {@link #zeroOrOne(int, int, int, int, int, int) } to make print or not
	 * decision.
	 * 
